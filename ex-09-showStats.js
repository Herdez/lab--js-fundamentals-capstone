...
function countEqualTo(number, list) {
	var result = list.filter(function(num){
		return num === number;
	});
...
function countInputGreaterThan(number, list) {
	var result = list.filter(function(num){
		return number > num;
	});
...
function countInputLessThan(number, list) {
	var result = list.filter(function(num){
		return number < num;
	});
...
function showStats(number, list) {
	var stat = {};
	stat["inputArraySize"] = list.length;
	stat["count_inputEqualTo"] = countEqualTo(number, list);
	stat["count_inputGreaterThan"] = countInputGreaterThan(number, list);
    stat["count_inputLessThan"] = countInputLessThan(number, list);
    return stat;
}
